...
class Transpiler(Transformer):
    ...
    def classdef(self, items):
        ...
        return ast.ClassDef(name=class_name,bases=[],keywords=[],body=body,decorator_list=[],type_params=[])
    ...
    def assign_stmt(self, items):
        ...
        return ast.Assign(targets=[transformed_targets], value=transformed_value)

    def expr_stmt(self, items):
        value = items[0]
        transformed_value = self.transform(value) if isinstance(value, Tree) else value
        return ast.Expr(value=transformed_value)
    ...
    def comparison(self, items):
        ...
        for i in range(1, len-1):
            if i%2==1:
                ...
                transformed_comparator.append(transformed_comparator)
        return ast.Compare(left=transformed_value_l,ops=transformed_ops,comparators=transformed_comparators)
    ...
    def or_expr(self, items):
        ...
        return ast.BoolOp(op=ast.Or(), values=transformed_values)
    ...
    def and_expr(self, items):
        ...
        transformed_value_r = self.transform(value_right) if isinstance(value_right, Tree) else value_right
        transformed_values = [transformed_value_l,transformed_value_r]
        return ast.BoolOp(op=ast.And(), values=transformed_values)
    ...
    def arith_expr(self, items):
        ...
        return ast.BinOp(transformed_value_l,transformed_op,transformed_value_r)
    ...
    def factor(self, items):
        ...
        if(transformed_value_l == "+"):
            ...
            op = ast.USub()
        return ast.UnaryOp(op,transformed_value_r)
    ...
    def if_stmt(self, items):
        ...
        return ast.If(transformed_test,transformed_then_body,transformed_else_body)
    ...
    def funccall(self, items):
        ...
        return ast.Call(func=transformed_func, args=args)
    ...
    def funccallwithversion(self, items):
        ...
        # バージョンの情報もクラス名が持つ
        func_name = str(func) + "_v_" + str(version)
        return ast.Call(func=func_name, args=args)
    ...
    def getattr(self, items):
        ...
        return ast.Attribute(value=transformed_value, attr=transformed_attr)
    ...
    def funcdef(self, items):
        ...
        if isinstance(params_tree, Tree):
            ...
            args = params_tree
        return ast.FunctionDef(name=name, args=args, body=self._flatten_list(body),decorator_list=[],type_params=[])
